...
class FnetSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FnetSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class FnetAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = FnetSelfAttention(config)
        self.output = FnetSelfOutput(config)
...
class FnetLayer(nn.Module):
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = FnetAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = FnetAttention(config)
...
class FnetLMHeadModel(FnetPreTrainedModel):
